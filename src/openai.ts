...
export class OpenAI {
  ...
  constructor(private apiUrl: string, private accessToken: string, private orgId?: string, private model = 'gpt-3.5-turbo', private temperature = 0) {
  ...
  async reviewCodeChange(change: string): Promise<string> {
    ...
    const data: ICompletion = { ...openAiCompletionsConfig, model: this.model, temperature: this.temperature };
    data.messages = [
      systemContent,
      suggestContent,
      {
