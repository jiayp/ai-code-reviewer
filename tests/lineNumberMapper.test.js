...

{
  ...
  // Expected: old=7 → new=6 (context shifted by +2 added lines before it in diff)
  assertEqual(mapping[7], 6, 'old=7 should map to new=6');
  
  // Expected: old=8 → new=7
  assertEqual(mapping[8], 7, 'old=8 should map to new=7');
}
...

{
  ...
  console.log('Corrected +70 to:', resultOutOfRange.lineNumber, '(corrected:', resultOutOfRange.corrected, ')');
...

{
  ...
  console.log('\nMapping:', JSON.stringify(buildOldToNewMapping(change), null, 2));
...

{
  ...
  // Old line 7 (context) should map to new line 6
  const pos7 = buildPositionForGitLab(7, 'context', change);
  assertEqual(pos7.old_line, 7, 'context old_line=7');
  assertEqual(pos7.new_line, 6, 'context new_line for old=7 should be 6 (shifted)');
...
+line65_new
 ...
 line67`;
  ...
  console.log('Mapping:', JSON.stringify(buildOldToNewMapping(change), null, 2));
  console.log('Added ranges:', JSON.stringify(getAddedOnlyNewLineRanges(change)));
  ...
  assertTrue(allAdded.includes(68), '+line65_new should be at new line 68');
}
